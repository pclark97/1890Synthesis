...
import pclark.data.cph1890.ICPSR_02896

class ICPSR_02896_for1890Spec extends FlatSpec with ShouldMatchers with GZipSource {
  
  "ICPSR 02896 1890 VarSetup" should "have the right number of agg vars" in {
      ICPSR_02896.numericAggVars.size should be === 198
  }
  
  it should "be able to read agg data vars for states" in {
    ICPSR_02896.readData.size should be === 51
  }
  ...
  it should "get the right population size for Minnesota" in {
    ...
    data(ICPSR_02896.minnesotaICP).apply(totalPopVar) should be === 1301826
  }

  it should "be able to get a table by name" in {
    val raceByNativityForAlabama = ICPSR_02896.getTableForState(ICPSR_02896.nt6, ICPSR_02896.alabamaICP)
    raceByNativityForAlabama.size should be === 8
    // "Native white males/for parents"
    raceByNativityForAlabama(ICPSR_02896.vars("nbwmfp").defaultKey) should be === 11483
